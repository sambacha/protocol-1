...
import { ArtistV2__factory } from '../typechain';
 ...
 */
export async function getAuthSignature({ chainId, deployerAddress, privateKey, provider }: CreateArtistWhiteListArgs) {
 ...
 */
export async function getTokensOfEdition(contractAddress: string, editionId: string, provider: Provider) {
...
const provider = new JsonRpcProvider(`https://eth-mainnet.alchemyapi.io/v2/${process.env.NEXT_PUBLIC_ALCHEMY_KEY}`);

(async () => {
  const tokenData = await getTokensOfEdition('0x3bF96aFe2291D76f2934350624080fAefEec9a46', '1', provider);
